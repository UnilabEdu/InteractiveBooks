...
def book_lab():
    ...
    print(page_num)
    if search:
        books = Book.query.filter(Book.project_name.ilike(f"%{search}%")).paginate(per_page=1,page=page_num, error_out=True)
    else:
        books = Book.query.paginate(per_page=1,page=page_num, error_out=True)
